...
class AE_BP(object):
    def __init__(self, in_dim=784, hidden_units=[512], latent_dim=256, lr=0.001):
        self.in_dim = in_dim
        ...
        with tf.name_scope('latent_in'):
            ...
            latent_in = tf.nn.relu(tf.add(tf.matmul(inputs, weights), biases))
        ...
        with tf.name_scope('latent_out'):
            ...
            latent_out = tf.nn.relu(tf.add(tf.matmul(latent_in, weights), biases))
    ...
    def _loss(self):
        bce = self._bernoulli_loglikelihood(self.x, self.reconstruct_x)
    ...
    def fit(self, x_train=None, x_valid=None, batch_size=100, n_epochs=30, lr=0.001):
        # prepare batch data
        batch_xs = []
